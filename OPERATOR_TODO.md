...
## OP-18, fund Rotor + Stator and set AGENT_KEYS in Vercel prod (multi-attestor by default)   [open]
- Date: 2026-06-15
- Blocks: the corroborating-attestor pass now wired into both live posting paths actually firing in production. Per explicit operator request ("all agents active by default"), `apps/web/lib/fulfill.ts` (self-serve) and `scripts/v2-streak-run.ts` (daily streak) now attest with Reflector (required primary) and then Rotor + Stator as best-effort corroborators, each independently re-running the deterministic reconciler and posting its own verifiable trace (single-model triple-run redundancy, NOT multi-model consensus). The code is conservative: it NEVER funds a key (the deployer/admin key stays off-limits to this layer) and skips any extra attestor whose wallet is missing from `AGENT_KEYS` or below `ATTEST_LOCK + 0.1 MNT`. So today, unless prod is set up, new claims still carry a single Reflector attestation (unchanged behavior).
- Need:
  1. Set `AGENT_KEYS` (reflector,rotor,stator private keys, comma-separated) in Vercel prod env so `fulfill.ts` can resolve Rotor/Stator. The daily streak (GitHub Action) needs the same secret set.
  2. Fund the Rotor and Stator wallets with enough MNT for standing operation (each attest locks 0.02 MNT plus gas; budget several days of runway, like Reflector). Top up when low; the streak's low-balance guard pauses Reflector but the extras self-skip on low balance.
- Half-done state: code merged + CI green; verified locally that an unfunded/missing extra is skipped without breaking the Reflector attestation. Not yet exercised end-to-end in prod because that needs the env + funding above. Note this supersedes the older OP-10 line "Multi-attestor N-run is intentionally NOT pursued" — the operator explicitly requested it; the honest framing (single-model triple-run, conditional on funding) is recorded in docs/REALITY-AUDIT.md.
- Plugboard note: Plugboard is the EXTERNAL Nous Hermes runtime Bombe did not write; it is intentionally NOT on the synchronous critical path of either posting flow. It attests as a periodic one-shot (scripts/hermes/attest.mjs, OP-14) on its own cadence and its wallet is funded separately. "All agents active by default" covers the three in-house attestors (Reflector/Rotor/Stator); Plugboard remains an independent external attestor by design.
- To resolve: set `AGENT_KEYS` in Vercel prod + the streak workflow secrets, fund Rotor + Stator, then tell the agent "OP-18 ready" (it will post a fresh self-serve claim and confirm three attestors land on /verify).
