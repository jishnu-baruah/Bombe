 ...
 * /claim/[id] — T-605
 *
 * Per-claim trace viewer.
 * - Claim header with tier Badge, asset, payload.
 * - Per-agent tabs (Reflector/Rotor/Stator/Plugboard/Human).
 * - Each tab: full step list (thought → action → observation), source hashes,
 *   final decision block, on-chain record (reasoningHash, sourcesHash, traceURI).
 * - verify-hash button: client-side recompute hashCanonical(trace) via @bombe/shared;
 *   compare to stored reasoningHash → ✓ match / ✗ mismatch (PRD §14.4).
 * - Plugboard: skill_hash display + epoch-snapshot diff placeholder.
 * - BLOCKED BY PROTOCOL chip for Plugboard's claim-D attempt.
 */
...
function AgentTabs({
  ...
  return (
    <div>
      ...
      {/* Active panel */}
      <AgentPanel agentId={activeAgent} claimId={claimId} />
    </div>
  );
}
...
function ClaimHeader({ id }: { id: string }) {
...
export default function ClaimPage({
  ...
  const { id } = use(params);
  const attestations = getAttestations(id);
